...
!macro customInstall
  ...
  IfFileExists "$0" 0 service_install_missing
    ...
    ${If} $4 != 0
      MessageBox MB_ICONSTOP "Focus Blocker installed, but the background service could not be registered. Log: $3"
      Abort
    ${EndIf}
    Goto service_install_done

  service_install_missing:
    MessageBox MB_ICONSTOP "Focus Blocker service installer was not found: $0"
    Abort
...
!macro customUnInstall
  DetailPrint "Uninstalling Focus Blocker background service..."
  ...
  IfFileExists "$0" 0 service_uninstall_done
    ...
    ExecWait '"$SYSDIR\cmd.exe" /d /s /c "set ELECTRON_RUN_AS_NODE=1&& ""$1"" ""$0"" ""--user-data"" ""$2"" ""--exec-path"" ""$1"" ""--log-file"" ""$3"""' $4
    ${If} $4 != 0
      MessageBox MB_ICONEXCLAMATION "Focus Blocker was removed, but the background service cleanup failed. Log: $3"
    ${EndIf}
